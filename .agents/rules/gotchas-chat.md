...
- **Hybrid AI Gateway/RovoDev mode**: `pnpm run dev` starts backend + frontend and supports AI Gateway-backed chat when credentials are configured. `pnpm run rovodev` adds RovoDev Serve (single-instance default; use `pnpm run rovodev -- 6` for full pool). Requests that explicitly select RovoDev return 503 when RovoDev Serve is unavailable.
