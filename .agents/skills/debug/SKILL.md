...
- Primary runtime log: `.tmp/symphony/runtime/log/symphony.log`
  - The wrapper passes `--logs-root .tmp/symphony/runtime/log` to upstream
    Symphony by default.
  - Includes orchestrator, agent runner, and Codex app-server lifecycle logs.
- Rotated runtime logs: `.tmp/symphony/runtime/log/symphony.log*`
...
rg -n "issue_id=<linear-uuid>" .tmp/symphony/runtime/log/symphony.log*
...
rg -n "Issue stalled|scheduling retry|turn_timeout|turn_failed|Codex session failed|Codex session ended with error" .tmp/symphony/runtime/log/symphony.log*
```
...
`.tmp/symphony/runtime/log/symphony.log` and keyed by `session_id`. Read them
as a lifecycle:
...
- Check rotated logs (`.tmp/symphony/runtime/log/symphony.log*`) before concluding data is missing.
