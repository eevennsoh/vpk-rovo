...
| Role                                              | Token                               | Hex         | Use                                                                                                 |
| ------------------------------------------------- | ----------------------------------- | ----------- | --------------------------------------------------------------------------------------------------- |
| Default canvas                                    | `surface`                           | `#FFFFFF`   | Page background, card fill                                                                          |
| Sunken canvas                                     | `surface-sunken`                    | `#F8F8F8`   | Wells for grouping (Kanban columns, content backdrops)                                              |
| Raised card                                       | `surface-raised`                    | `#FFFFFF`   | Movable cards — MUST pair with `shadow-raised`                                                      |
| Overlay                                           | `surface-overlay`                   | `#FFFFFF`   | Modals, dropdowns, flags, popovers — MUST pair with `shadow-overlay`                                |
| Primary text                                      | `text`                              | `#292A2E`   | Paragraphs, list items, descriptions, titles, table cells, button labels — **all continuous prose** |
| Secondary text                                    | `text-subtle`                       | `#505258`   | Field labels, captions, bylines, nav items, metadata — **never paragraph text**                     |
| Tertiary text                                     | `text-subtlest`                     | `#6B6E76`   | Placeholders, helper text, breadcrumb separators, timestamps                                        |
| Inverse text                                      | `text-inverse`                      | `#FFFFFF`   | Text on bold backgrounds                                                                            |
| Disabled text                                     | `text-disabled`                     | `#080F214A` | Any disabled state                                                                                  |
| Icon                                              | `icon`                              | `#292A2E`   | Primary icons, paired with `text`                                                                   |
| Subtle icon                                       | `icon-subtle`                       | `#505258`   | Secondary icons                                                                                     |
| Subtlest icon                                     | `icon-subtlest`                     | `#6B6E76`   | Paired with `text-subtlest`                                                                         |
| Inverse icon                                      | `icon-inverse`                      | `#FFFFFF`   | Icons on bold backgrounds                                                                           |
| Default border                                    | `border`                            | `#0B120E24` | Dividers, input outlines, cards                                                                     |
| Strong border                                     | `border-bold`                       | `#7D818A`   | When 3:1 non-text contrast is required                                                              |
| Input border                                      | `border-input`                      | `#8C8F97`   | Form fields                                                                                         |
| Input background                                  | `background-input`                  | `#FFFFFF`   | Form fields                                                                                         |
...
| Neutral hovered                                   | `background-neutral-subtle-hovered` | `#0515240F` | Hover state for subtle controls                                                                     |
| Link                                              | `link`                              | `#1868DB`   | Navigational and textual links                                                                      |
| Selected surface                                  | `background-selected`               | `#E9F2FE`   | Active tabs, selected list rows                                                                     |
| Selected border                                   | `border-selected`                   | `#1868DB`   | Paired with `border-width-selected` (2px) for selection                                             |
| Selected text                                     | `text-selected`                     | `#1868DB`   | Text in active/selected state                                                                       |
| Focus ring                                        | `border-focused`                    | `#4688EC`   | Paired with `border-width-focused` (2px)                                                            |
...
| --------------- | ------------------------------------ | ------------------ | ------------------ | ----------------------------- | -------------------- | ----------- | ------------ |
| **brand**       | Primary CTAs, brand moments          | `text-brand`       | `icon-brand`       | `background-brand-bold`       | `border-brand`       | `#1868DB`   | `#FFFFFF`    |
| **danger**      | Destructive actions, blocking errors | `text-danger`      | `icon-danger`      | `background-danger-bold`      | `border-danger`      | `#C9372C`   | `#FFFFFF`    |
| **warning**     | Caution, non-blocking issues         | `text-warning`     | `icon-warning`     | `background-warning-bold`     | `border-warning`     | `#FBC828`   | `#292A2E`    |
| **success**     | Positive outcomes, completion        | `text-success`     | `icon-success`     | `background-success-bold`     | `border-success`     | `#5B7F24`   | `#FFFFFF`    |
| **information** | Neutral info, in-progress            | `text-information` | `icon-information` | `background-information-bold` | `border-information` | `#1868DB`   | `#FFFFFF`    |
| **discovery**   | New features, beta, change           | `text-discovery`   | `icon-discovery`   | `background-discovery-bold`   | `border-discovery`   | `#964AC0`   | `#FFFFFF`    |
...
| DO                                                                                                                                                                                  | DON'T                                                                                                                                                         |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Hex anchors mapped to a token (e.g. `text` → `#292A2E`)                                                                                                                             | Arbitrary one-off hex (`color: '#172B4D'`) with no token mapping                                                                                              |
...
`fill="currentcolor"` — set the parent element's `color` to an `icon-*` hex (e.g. `color: #C9372C`
...
```html
<!-- meaningful icon: give it an accessible name. color = icon-danger (#C9372C) -->
<span style="color: #C9372C; display: inline-flex;">
...
<!-- background = background-accent-blue-subtler (#CFE1FD), color = icon-accent-blue (#357DE8) -->
<span
	role="img"
	aria-label="Add"
	style="
...
> Build a title slide for an internal Atlassian deck. Background: `surface` (`#FFFFFF`).
> Left-aligned within `space-800` margins. Title in Charlie Display 40pt bold, `text` (`#292A2E`).
> Subtitle in Atlassian Sans 18pt regular, `text-subtle` (`#505258`). Single accent: a 4px-tall
> horizontal rule in `text-brand` (`#1868DB`) below the title, 64px wide. Atlassian wordmark
