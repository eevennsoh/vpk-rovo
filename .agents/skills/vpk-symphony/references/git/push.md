...
2. Run local validation before pushing. Follow `AGENTS.md`: targeted tests for
   touched code, and `pnpm run lint` plus `pnpm run typecheck` when the issue,
   touched surface, or reviewer feedback requires broad repo validation.
...
6. Write/update the PR body explicitly:
   - If `.github/pull_request_template.md` exists, fill every section with
     concrete content and replace placeholder comments (`<!-- ... -->`).
   - If no template exists, use concise `Summary` and `Validation` sections.
   - If PR already exists, refresh body content so it reflects the total PR
     scope (all intended work on the branch), not just the newest commits,
     including newly added work, removed work, or changed approach.
   - Do not reuse stale description text from earlier iterations.
7. Re-read the PR body before handoff and make sure the validation section
   exactly matches commands or manual checks that actually ran.
...
# Minimal validation gate: choose commands from AGENTS.md for touched files.
# Common broad checks:
pnpm run lint
pnpm run typecheck
...
# Write/edit PR body before handoff.
# If no .github/pull_request_template.md exists, use:
# Summary:
# - <change>
#
# Validation:
