...
### 2026-03-21 - Codex CLI version ceiling is 0.107.0

- **What happened:** Codex CLI 0.110.0+ introduces a "failed to load your workspace-managed config" error. The specific change is PR #13388 (`config: enforce enterprise feature requirements`) merged into 0.110.0. The CLI detects `chatgpt_plan_type: "business"` from the JWT token, tries to fetch cloud requirements from OpenAI servers, and fails closed when no workspace-managed config is published for the org. There is no env var or config flag to bypass this — it's hardcoded.
- **Why:** The account authenticates as ChatGPT Business plan with a "Personal" org, which has no published cloud `requirements.toml`. The CLI's fail-closed design means it refuses to start.
- **Rule:** Keep Codex CLI at **0.107.0** (installed via `npm install -g @openai/codex@0.107.0`). Do not upgrade past 0.107.0. If Homebrew installs a newer version, uninstall it and use the npm global install instead. The cloud `requirements.toml` only accepts policy constraint fields (allowed lists, sandbox, otel, mcp server identities, command rules) — not full config mirrors (model, features, personality).

### 2026-03-23 - Reuse the active Atlassian site URL before hardcoding Confluence defaults

- **What happened:** The backend default Confluence base URL was left at `https://venn-test.atlassian.net/wiki` in this worktree even though the active local site and Rovo configuration use `https://hello.atlassian.net`.
- **Why:** A repo-local constant was edited without cross-checking it against the currently configured site URL already present in the running environment and surrounding configuration.
- **Rule:** Before hardcoding or changing an Atlassian base URL, verify the active site from the local configuration and existing runtime defaults in the repo. Prefer reusing the current configured site instead of introducing a stale environment-specific URL.

### 2026-03-23 - Keep Jira defaults aligned with the product-fabric site

- **What happened:** A follow-up correction clarified that any default Jira base URL in this repo must target `https://product-fabric.atlassian.net/`.
