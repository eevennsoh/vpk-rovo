...
test("POST /api/rovo/cancel-deferred-tool rejects malformed JSON as a client error", async (t) => {
	const { POST } = await loadBundledRoute(t);

	const response = await POST(new Request("http://localhost/api/rovo/cancel-deferred-tool", {
		body: "{",
