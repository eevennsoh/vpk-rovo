...
function normalizeGeneratedFileEntry(rawEntry) {
	...
	const workspacePath =
		buildFutureChatWorkspaceRelativePath(requestedPath)
...
function extractGeneratedFilesFromObservation(observation) {
	...
	return [...foundPaths].map((requestedPath) =>
		normalizeGeneratedFileEntry({
			requestedPath,
			workspacePath: buildFutureChatWorkspaceRelativePath(requestedPath),
...
function createFutureChatGeneratedFilesManager({
	baseDir,
	projectRoot,
	logger = console,
} = {}) {
	...
	const readManifest = async (threadId) => {
		const { generatedFilesPath } = buildFutureChatThreadPaths(baseDir, threadId);
	...
	const moveRootFileToWorkspace = async (threadId, requestedPath) => {
		...
		const destinationPath = buildFutureChatWorkspacePath(baseDir, threadId, normalizedPath);
		if (!destinationPath) {
			return false;
		}
