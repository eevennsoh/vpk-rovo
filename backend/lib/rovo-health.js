...
function classifyRovoHealthCheck(result) {
	...
	if (result.requiresAuth === true) {
		return {
			...
			message: "Rovo Serve requires authentication before it can accept normal traffic.",
		};
	}
	...
	if (status === "pending user review" || status === "entitlement check failed") {
		return {
			...
			message:
				status === "pending user review"
					? "Rovo Serve is waiting for user review."
					: "Rovo Serve entitlement checks failed.",
			detail,
			mcpServers,
		};
	}
	...
	if (RETRYABLE_HEALTH_STATUSES.has(status)) {
		return {
			...
			message:
				status === "unknown"
					? "Rovo Serve is still starting."
					: "Rovo Serve reported an unhealthy state.",
			detail,
			mcpServers,
		};
	}
	...
	return {
		...
		message: "Rovo Serve returned an unrecognized health state.",
		detail,
		mcpServers,
	};
}
