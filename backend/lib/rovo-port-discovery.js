...
function readRecordedRovoPorts({
	portFile,
	portsFile,
	fsModule = fs,
} = {}) {
...
function writeRecordedRovoPorts({
	ports,
	portFile,
	portsFile,
	fsModule = fs,
...
function buildRovoDiscoveryCandidatePorts({
	envPort,
	basePort,
	maxTries = 20,
} = {}) {
...
async function discoverHealthyRovoPorts({
	candidatePorts,
	healthCheck,
	classifyHealthCheck,
} = {}) {
	if (typeof healthCheck !== "function") {
		throw new Error("discoverHealthyRovoPorts requires healthCheck");
	}
...
async function resolveRovoPorts({
	portFile,
	portsFile,
	envPort,
	basePort,
	...
	const recordedPorts = readRecordedRovoPorts({
		portFile,
		portsFile,
		fsModule,
	});
	...
	const candidatePorts = buildRovoDiscoveryCandidatePorts({
		envPort,
		basePort,
		maxTries,
	});
	...
	const discoveredPorts = await discoverHealthyRovoPorts({
		candidatePorts,
		healthCheck,
		classifyHealthCheck,
	});
...
module.exports = {
	...
	discoverHealthyRovoPorts,
	parsePositiveInteger,
	readRecordedRovoPorts,
	resolveRovoPorts,
	writeRecordedRovoPorts,
};
