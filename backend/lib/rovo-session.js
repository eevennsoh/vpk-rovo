...
async function createRovoSession(port, { customTitle, timeoutMs = 10_000 } = {}) {
	const body =
...
async function getCurrentRovoSession(port, { timeoutMs = 10_000 } = {}) {
	return requestSessionJson("GET", "/v3/sessions/current_session", undefined, timeoutMs, port);
}

async function restoreRovoSession(port, sessionId, { timeoutMs = 10_000 } = {}) {
	const normalizedSessionId = normalizeSessionId(sessionId);
	if (!normalizedSessionId) {
		throw new Error("A non-empty sessionId is required to restore a Rovo session.");
	}
...
async function ensureRovoSession(port, {
	sessionId,
	customTitle,
	timeoutMs = 10_000,
} = {}) {
	...
	if (normalizedSessionId) {
		try {
			return await restoreRovoSession(port, normalizedSessionId, { timeoutMs });
		} catch (error) {
			const status =
	...
	return createRovoSession(port, { customTitle, timeoutMs });
}
