...
test("createRovoSession posts the documented create payload", async () => {
	const requests = [];
	...
	try {
		const result = await createRovoSession(port, {
...
test("restoreRovoSession posts the documented restore payload", async () => {
	const requests = [];
	const server = http.createServer(async (req, res) => {
		requests.push(`${req.method} ${req.url}`);
	...
	try {
		const result = await restoreRovoSession(port, "session-2");
...
test("getCurrentRovoSession normalizes the current session payload", async () => {
	const requests = [];
	const server = http.createServer(async (req, res) => {
		requests.push(`${req.method} ${req.url}`);
	...
	try {
		const result = await getCurrentRovoSession(port);
...
test("getCurrentRovoSession falls back to X-Session-ID response header", async () => {
	const requests = [];
	const server = http.createServer(async (req, res) => {
		requests.push(`${req.method} ${req.url}`);
		if (req.method === "GET" && req.url === "/v3/sessions/current_session") {
...
test("ensureRovoSession falls back to session creation when restore returns 404", async () => {
	const requests = [];
	const server = http.createServer(async (req, res) => {
		requests.push(`${req.method} ${req.url}`);
	...
	try {
		const result = await ensureRovoSession(port, {
			sessionId: "session-4",
