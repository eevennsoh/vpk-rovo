...
export function AgentSessionColumnHeaderActionToggles({
	showFilter,
	showOverflow,
	onShowFilterChange,
	onShowOverflowChange,
}: Readonly<{
	showFilter: boolean;
	showOverflow: boolean;
	onShowFilterChange: (next: boolean) => void;
	onShowOverflowChange: (next: boolean) => void;
}>) {
	return (
		<div className="flex items-center gap-1" role="group" aria-label="Column header actions">
			<Toggle
				aria-label="Show filter"
				onPressedChange={onShowFilterChange}
				pressed={showFilter}
				size="sm"
				variant="outline"
			>
			...
			<Toggle
				...
				onPressedChange={onShowOverflowChange}
				pressed={showOverflow}
				size="sm"
				variant="outline"
			>
				Overflow
...
export default function AgentSessionColumnPage({
	...
	return (
		<div className="flex h-full w-full flex-col gap-10 bg-surface p-6">
			<DemoSection action={headerActionToggles} label="Panel">
			...
			>
				<div className="min-h-[560px] h-[640px] overflow-hidden">
					<ExperimentalJiraKanbanPage
						...
						showAgentSessionFilter={showFilter}
						showAgentSessionOverflow={showOverflow}
					/>
				</div>
			</DemoSection>
		</div>
