...
test("work item modal exposes the agent panel under child work items", () => {
	...
	assert.match(panelSource, /Manage/);
	...
	assert.match(panelSource, /\{shouldSuggestVoiceMate \? "Manage" : "Start work"\}[\s\S]*<ChevronDownIcon label="" size="small" \/>/);
	...
	assert.match(panelSource, /next\/image/);
	assert.match(panelSource, /RFP_DRAFTING_AGENT_NAME/);
	assert.match(panelSource, /Suggested for this HTML outline/);
	...
	assert.match(panelSource, /shouldSuggestVoiceMate \? "items-stretch justify-between gap-3" : "items-center justify-center"/);
	...
	assert.match(panelSource, /<DropdownMenuTrigger[\s\S]*aria-label=\{`Open agent selector for \$\{workItem\.code\}`\}[\s\S]*shouldSuggestVoiceMate \? "Manage" : "Start work"/);
	assert.match(panelSource, /<DropdownMenuContent[\s\S]*className="w-\[360px\] overflow-hidden p-0"[\s\S]*positionerClassName="z-\[502\]"/);
	assert.match(panelSource, /<AgentSelector[\s\S]*agents=\{agents\}[\s\S]*onAgentToggle=\{handleAgentToggle\}[\s\S]*onBrowseAgents=\{handleFooterAction\}[\s\S]*onCreateAgent=\{handleFooterAction\}/);
