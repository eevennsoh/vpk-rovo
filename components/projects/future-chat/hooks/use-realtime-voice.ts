...
} from "@/components/projects/future-chat/lib/future-chat-realtime-assistant-state";
import {
...
} from "@/components/projects/future-chat/lib/future-chat-voice-capture";
...
export function useRealtimeVoice({
	...
	const hasCompletedActiveSpeechTurn = useCallback((transcript: string) => {
		return hasCompletedFutureChatVoiceTurn({
			activeTurnId: activeSpeechTurnIdRef.current,
			completedTurnId: completedSpeechTurnRef.current?.turnId ?? null,
	...
	const markActiveSpeechTurnCompleted = useCallback((transcript: string) => {
		...
		completedSpeechTurnRef.current = {
			transcript: normalizeFutureChatVoiceTranscript(transcript),
			turnId,
		};
		return turnId;
	...
	const resolveCaptureAvailability = useCallback((): boolean => {
		...
		return activeRef.current && isFutureChatVoiceCaptureAvailable({
	...
	const scheduleBrowserTranscriptCompletionFallback = useCallback((transcript: string) => {
		const normalizedTranscript = normalizeFutureChatVoiceTranscript(transcript);
