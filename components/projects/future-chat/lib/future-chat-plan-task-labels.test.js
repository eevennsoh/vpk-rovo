...
const {
	resolveFutureChatPlanTaskDisplayLabels,
} = require("./future-chat-plan-task-labels.ts");
const {
...
test("resolveFutureChatPlanTaskDisplayLabels uses plan labels when present (backward compat)", () => {
	const planWidget = {
...
test("resolveFutureChatPlanTaskDisplayLabels uses exact latest update_todo labels after approval", () => {
	const planWidget = {
		title: "Jira Analytics Dashboard",
		description: "Build the dashboard route",
		markdown: "## Plan",
