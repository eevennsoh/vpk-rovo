...
export function normalizeFutureChatPlanTaskIdAlias(taskId: string): string {
...
export function futureChatPlanTaskIdsMatch(
	leftTaskId: string,
	rightTaskId: string,
): boolean {
	return (
		normalizeFutureChatPlanTaskIdAlias(leftTaskId) ===
		normalizeFutureChatPlanTaskIdAlias(rightTaskId)
	);
}

export function getLatestFutureChatTodoProgressFromMessages(
	messages: ReadonlyArray<RovoUIMessage>,
...
export function buildFutureChatPlanTaskDisplayLabels(
...
export function resolveFutureChatPlanTaskDisplayLabels(input: {
	messages: ReadonlyArray<RovoUIMessage>;
	...
	if (!planKey) {
		return buildFutureChatPlanTaskDisplayLabels(planWidget.tasks, null);
	}
	...
	const latestSnapshot =
		getLatestFutureChatTodoProgressFromMessages(executionMessages);

	return buildFutureChatPlanTaskDisplayLabels(planWidget.tasks, latestSnapshot);
}
