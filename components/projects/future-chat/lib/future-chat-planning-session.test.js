...
} = require("./future-chat-planning-session.ts");
...
test("getLatestFutureChatAssistantMessageId returns the latest assistant id", () => {
	assert.equal(
		getLatestFutureChatAssistantMessageId([
...
test("planning artifacts ignore older plan cards before the baseline assistant message", () => {
	const result = getFutureChatPlanningArtifactsSinceBaseline(
		[
