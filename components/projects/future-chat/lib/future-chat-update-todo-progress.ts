...
): FutureChatTodoProgressItem | null {
...
function dedupeTodoItems(
	...
	const dedupedItems: FutureChatTodoProgressItem[] = [];
...
export function getLatestFutureChatTodoProgress(
	...
	for (let index = toolCalls.length - 1; index >= 0; index -= 1) {
		...
		const parsed =
			parseFutureChatTodoProgressFromText(toolCall.output) ??
			parseFutureChatTodoProgressFromText(toolCall.outputPreview);
		if (parsed) {
			return parsed;
		}
	}
