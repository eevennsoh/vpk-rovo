...
test("createListWorkItemFromSession mints a To-do card titled from the session and attaches the activity", () => {
	...
	assert.equal(todoCard?.assignee?.id, JIRA_GOLDEN_JOURNEYS_V4_PAY_CURRENT_USER.id);
	assert.equal(todoCard?.assignee?.name, JIRA_GOLDEN_JOURNEYS_V4_PAY_CURRENT_USER.name);
	assert.equal(todoCard?.avatarSrc, JIRA_GOLDEN_JOURNEYS_V4_PAY_CURRENT_USER.avatarSrc);
...
test("createBoardWorkItemFromSession appends a task to the requested status and attaches its session", () => {
	...
	assert.equal(createdCard?.assignee?.id, JIRA_GOLDEN_JOURNEYS_V4_PAY_CURRENT_USER.id);
	assert.equal(createdCard?.assignee?.name, JIRA_GOLDEN_JOURNEYS_V4_PAY_CURRENT_USER.name);
	assert.equal(createdCard?.avatarSrc, JIRA_GOLDEN_JOURNEYS_V4_PAY_CURRENT_USER.avatarSrc);
