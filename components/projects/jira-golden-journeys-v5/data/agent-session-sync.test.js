...
const HOOK_SOURCE = readFileSync(
	path.join(__dirname, "../hooks/use-jira-golden-journeys-v5-agent-session-sync.ts"),
	"utf8",
);
	...
	if (!syncModulePromise) {
		syncModulePromise = esbuild
			...
			.then((result) => loadCjsModuleFromText(
				...
				"jira-golden-journeys-v5-agent-session-sync-harness.cjs",
			));
	}
...
test("the Jira v5 demo syncs one or two new sessions per batch", async () => {
	...
	assert.equal(sync.takeJiraGoldenJourneysV5SyncBatch(0, () => 0).sessions.length, 1);
	assert.equal(sync.takeJiraGoldenJourneysV5SyncBatch(0, () => 0.999).sessions.length, 2);

	const finalBatch = sync.takeJiraGoldenJourneysV5SyncBatch(
		sync.JIRA_GOLDEN_JOURNEYS_V5_SYNC_SESSIONS.length - 1,
		() => 0.999,
	);
	assert.equal(finalBatch.sessions.length, 1);
	assert.equal(finalBatch.nextIndex, sync.JIRA_GOLDEN_JOURNEYS_V5_SYNC_SESSIONS.length);
});

test("the Jira v5 demo chooses a fresh delay inside the four-to-eight-second window", async () => {
	const sync = await loadSyncModule();

	assert.equal(sync.getJiraGoldenJourneysV5SyncDelayMs(() => 0), 4_000);
	assert.equal(sync.getJiraGoldenJourneysV5SyncDelayMs(() => 0.5), 6_000);
	assert.equal(sync.getJiraGoldenJourneysV5SyncDelayMs(() => 0.999_999), 8_000);
});

test("every queued Jira v5 session has a unique stable identity", async () => {
	const sync = await loadSyncModule();
	const sessions = sync.JIRA_GOLDEN_JOURNEYS_V5_SYNC_SESSIONS;
...
test("reviewing synced sessions clears all or only the named arrival marks", async () => {
	...
	assert.deepEqual(
		[...sync.removeReviewedJiraGoldenJourneysV5AgentSessionIds(current, ["second"])],
		["first", "third"],
	);
	assert.deepEqual(
		[...sync.removeReviewedJiraGoldenJourneysV5AgentSessionIds(current)],
		[],
	);
});
...
test("the route periodically syncs one or two new agent sessions into Untracked work", () => {
	assert.match(
		PAGE_SOURCE,
		/import \{ useJiraGoldenJourneysV5AgentSessionSync \} from "\.\/hooks\/use-jira-golden-journeys-v5-agent-session-sync";/u,
	);
	assert.match(
		PAGE_SOURCE,
		/const \{\s*reviewAgentSessions,\s*newAgentSessionIds,\s*syncedAgentSessions,\s*\} = useJiraGoldenJourneysV5AgentSessionSync\(\{ active: showBoardContent \}\);/u,
	);
	assert.match(
		PAGE_SOURCE,
	...
	assert.match(HOOK_SOURCE, /removeReviewedJiraGoldenJourneysV5AgentSessionIds/u);
});
