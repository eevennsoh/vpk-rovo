...
type JiraGoldenJourneysV5SyncSession = Extract<PulseLooseWork, { kind: "agent-session" }>;

const SYNC_DELAY_MIN_MS = 4_000;
const SYNC_DELAY_MAX_MS = 8_000;

export const JIRA_GOLDEN_JOURNEYS_V5_SYNC_SESSIONS = [
	{
		agentId: "cursor",
...
] as const satisfies readonly JiraGoldenJourneysV5SyncSession[];

export function getJiraGoldenJourneysV5SyncDelayMs(
	random: () => number = Math.random,
): number {
	return SYNC_DELAY_MIN_MS + Math.round(random() * (SYNC_DELAY_MAX_MS - SYNC_DELAY_MIN_MS));
}

export function takeJiraGoldenJourneysV5SyncBatch(
	nextIndex: number,
	random: () => number = Math.random,
): Readonly<{
	nextIndex: number;
	sessions: readonly JiraGoldenJourneysV5SyncSession[];
}> {
	const batchSize = random() < 0.5 ? 1 : 2;
	const sessions = JIRA_GOLDEN_JOURNEYS_V5_SYNC_SESSIONS.slice(
		nextIndex,
		nextIndex + batchSize,
	);
...
export function removeReviewedJiraGoldenJourneysV5AgentSessionIds(
