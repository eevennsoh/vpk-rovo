	...
	if (!presentationModulePromise) {
		presentationModulePromise = esbuild
			...
			.then((result) => loadCjsModuleFromText(
				...
				"jira-golden-journeys-v5-presentation-story-harness.cjs",
			));
	}
...
test("the presentation chapters follow the manager journey from Track to Terminal", async () => {
	...
	assert.deepEqual(
		story.JIRA_GOLDEN_JOURNEYS_V5_PRESENTATION_CHAPTERS,
		[
...
test("the PAY board fills every existing status with coding work and the full state matrix", async () => {
	const story = await loadPresentationModule();
	const columns = story.createJiraGoldenJourneysV5PayBoardColumns();
	const cards = columns.flatMap((column) => column.cards);

	assert.deepEqual(
		[...story.JIRA_GOLDEN_JOURNEYS_V5_PAY_STATUS_PHASES],
	...
	assert.ok(story.JIRA_GOLDEN_JOURNEYS_V5_PAY_BOARD_AGENTS.every((agent) => (
	...
	assert.deepEqual(
		story.JIRA_GOLDEN_JOURNEYS_V5_PAY_HEADER_ASSIGNEES.map((assignee) => assignee.id),
		["venn", "review-agent", "test-agent", "release-agent"],
	);
	assert.deepEqual(
		story.JIRA_GOLDEN_JOURNEYS_V5_PAY_HEADER_ASSIGNEES.map((assignee) => assignee.name),
		["Venn", "Codex", "Cursor", "GitHub Copilot"],
	);
	assert.ok(story.JIRA_GOLDEN_JOURNEYS_V5_PAY_HEADER_ASSIGNEES.every((assignee) => (
...
test("a linked Jira activity becomes a medium-detached Agent Session item", async () => {
	const story = await loadPresentationModule();
	const card = story.createJiraGoldenJourneysV5PayBoardColumns()
	...
	assert.ok(card);
	assert.ok(activity);
	const detached = story.toJiraGoldenJourneysV5DetachedAgentSession(activity, card);
	assert.deepEqual(
		detached,
		{
			id: activity.id,
	...
	assert.deepEqual(
		story.toJiraGoldenJourneysV5AgentActivityFromSession(detached),
		{
			id: activity.id,
			name: activity.name,
			avatarSrc: activity.avatarSrc,
...
test("the board factory returns isolated cards and nested agent state", async () => {
	const story = await loadPresentationModule();
	const first = story.createJiraGoldenJourneysV5PayBoardColumns();
	const second = story.createJiraGoldenJourneysV5PayBoardColumns();
...
test("PAY-101 Build captures the inventory agent run and the first Insight's merged evidence", async () => {
	const story = await loadPresentationModule();
	const state = story.createJiraGoldenJourneysV5Pay101BuildState();

	assert.equal(story.JIRA_GOLDEN_JOURNEYS_V5_PAY_101_WORK_ITEM.code, "PAY-101");
	assert.match(story.JIRA_GOLDEN_JOURNEYS_V5_PAY_101_WORK_ITEM.title, /Inventory every v1 call site/u);
	assert.equal(state.activeSessionId, null, "Build should keep the initial Activity viewport at the top");
	assert.equal(state.sessions.length, 1);
	assert.equal(state.sessions[0].id, story.JIRA_GOLDEN_JOURNEYS_V5_PAY_101_SESSION_ID);
	assert.equal(story.JIRA_GOLDEN_JOURNEYS_V5_PAY_101_UNCAPTURED_SESSION_ID, "lw-scope-thread");
	assert.notEqual(
		state.sessions[0].id,
		story.JIRA_GOLDEN_JOURNEYS_V5_PAY_101_UNCAPTURED_SESSION_ID,
...
test("the PAY-specific agent exports cover the board and Build composer without SHOP aliases", async () => {
	...
	assert.ok(story.JIRA_GOLDEN_JOURNEYS_V5_PAY_BOARD_AGENTS.length >= 4);
	assert.ok(story.JIRA_GOLDEN_JOURNEYS_V5_PAY_COMPOSER_AGENTS.length >= 4);
	assert.ok(story.JIRA_GOLDEN_JOURNEYS_V5_PAY_BOARD_AGENTS.some((agent) => agent.id === "claude-code"));
	assert.ok(story.JIRA_GOLDEN_JOURNEYS_V5_PAY_COMPOSER_AGENTS.some((agent) => agent.id === "claude-code"));
});
