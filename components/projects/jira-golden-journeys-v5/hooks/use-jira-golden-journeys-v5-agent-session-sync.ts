...
import {
	getJiraGoldenJourneysV5SyncDelayMs,
	JIRA_GOLDEN_JOURNEYS_V5_SYNC_SESSIONS,
	removeReviewedJiraGoldenJourneysV5AgentSessionIds,
	takeJiraGoldenJourneysV5SyncBatch,
} from "../data/agent-session-sync";

interface JiraGoldenJourneysV5AgentSessionSyncState {
	newAgentSessionIds: ReadonlySet<string>;
	nextIndex: number;
	syncedAgentSessions: readonly PulseAgentSession[];
}

function createInitialSyncState(): JiraGoldenJourneysV5AgentSessionSyncState {
	return {
...
export function useJiraGoldenJourneysV5AgentSessionSync({
	active,
	...
	const reviewAgentSessions = useCallback((sessionIds?: readonly string[]) => {
		setSyncState((current) => {
			const newAgentSessionIds = removeReviewedJiraGoldenJourneysV5AgentSessionIds(
	...
	useEffect(() => {
		if (!active || syncState.nextIndex >= JIRA_GOLDEN_JOURNEYS_V5_SYNC_SESSIONS.length) {
			return undefined;
		}
		...
		const scheduleNextSync = () => {
			...
			timeoutId = window.setTimeout(() => {
				timeoutId = undefined;
				const batch = takeJiraGoldenJourneysV5SyncBatch(syncState.nextIndex);
