...
import { progressJiraGoldenJourneysV5WorkItemOnStart } from "@/components/projects/jira-golden-journeys-v5/lib/list-rows";

interface UseJiraGoldenJourneysV5GenerativeActionsOptions {
...
export function useJiraGoldenJourneysV5GenerativeActions({
	openAgentChat,
	setBoardColumns,
}: Readonly<UseJiraGoldenJourneysV5GenerativeActionsOptions>) {
	...
	const handleCardGenerativeActionSubmit = useCallback((
		...
		if ((request.kind === "agent" || request.kind === "skill") && request.selectedItem) {
			...
			setBoardColumns((columns) => progressJiraGoldenJourneysV5WorkItemOnStart(
