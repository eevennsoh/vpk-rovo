...
import { JIRA_GOLDEN_JOURNEYS_V5_PAY_BOARD_AGENTS } from "../data/presentation-story";
import {
	...
	insertListOrderKey,
	insertWorkItemCard,
	JIRA_GOLDEN_JOURNEYS_V5_LIST_STATUS_OPTIONS,
	moveListOrder,
...
export interface UseJiraGoldenJourneysV5ListResult {
...
export function useJiraGoldenJourneysV5List({
	boardColumns,
...
}>): UseJiraGoldenJourneysV5ListResult {
	...
	const getProps = useCallback((columns: readonly JiraKanbanColumnData[]): JiraListProps => {
		const rows = applyListOrder(
			createListRows(columns, JIRA_GOLDEN_JOURNEYS_V5_PAY_BOARD_AGENTS),
			listOrder,
		);
		...
		return {
			agentCatalog: JIRA_GOLDEN_JOURNEYS_V5_PAY_BOARD_AGENTS,
			...
			rows,
			selectedIssueKeys,
			statusOptions: JIRA_GOLDEN_JOURNEYS_V5_LIST_STATUS_OPTIONS,
