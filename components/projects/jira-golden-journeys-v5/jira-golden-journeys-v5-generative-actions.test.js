...
const GENERATIVE_ACTIONS_HOOK_SOURCE = readProjectFile(
	"components/projects/jira-golden-journeys-v5/hooks/use-jira-golden-journeys-v5-generative-actions.ts",
);
...
test("card agent and skill actions start a coding-agent session and stage the selected skill in floating chat", () => {
	...
	assert.match(PAGE_SOURCE, /useJiraGoldenJourneysV5GenerativeActions\(\{[\s\S]*openAgentChat,[\s\S]*setBoardColumns,[\s\S]*\}\)/u);
