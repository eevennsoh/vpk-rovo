...
const PAGE_SOURCE = readProjectFile("components/projects/jira-golden-journeys-v5/page.tsx");
const LIST_HOOK_SOURCE = readProjectFile(
	"components/projects/jira-golden-journeys-v5/hooks/use-jira-golden-journeys-v5-list.ts",
);
...
const INDICATORS_SOURCE = readProjectFile(
	"components/projects/jira-golden-journeys-v5/data/agent-activity-indicators.tsx",
);
...
test("the route renders the Payments board directly inside Jira app chrome", () => {
	...
	assert.match(PAGE_SOURCE, /createJiraGoldenJourneysV5PayBoardColumns/u);
	assert.match(PAGE_SOURCE, /JIRA_GOLDEN_JOURNEYS_V5_PAY_BOARD_AGENTS/u);
	assert.match(PAGE_SOURCE, /JIRA_GOLDEN_JOURNEYS_V5_PAY_HEADER_ASSIGNEES/u);
...
test("chin-row agent activity indicators use the Team EU renderer", () => {
	...
	assert.match(
		INDICATORS_SOURCE,
		/renderJiraGoldenJourneysV5AgentActivityIndicator[\s\S]*state === "awaiting-input" \? \(\s*<QuestionCircleFilledIcon color=\{token\("color\.icon\.information"\)\} label="" size="small" \/>\s*\) : \(\s*<Spinner label="" pulse size="default" variant="experimental" \/>\s*\)/u,
	);
	...
	assert.match(
		INDICATORS_SOURCE,
		/renderJiraGoldenJourneysV5AgentActivityIndicator[\s\S]*if \(state === "finished"\) \{\s*return <StatusSuccessIcon color=\{token\("color\.icon\.success"\)\} label="" size="small" \/>;\s*\}/u,
	);
	...
	assert.match(
		PAGE_SOURCE,
		/import \{ renderJiraGoldenJourneysV5AgentActivityIndicator \} from "\.\/data\/agent-activity-indicators";/u,
	);
	assert.match(
		PAGE_SOURCE,
		/<ExperimentalJiraKanbanPage[\s\S]*renderAgentActivityIndicator=\{renderJiraGoldenJourneysV5AgentActivityIndicator\}/u,
	);
...
test("unlinked agent sessions remain detached beneath their source Jira card", () => {
	...
	assert.match(PAGE_SOURCE, /toJiraGoldenJourneysV5DetachedAgentSession\(activity, card\)/u);
...
test("the Work items header switches between Board and List views with their icons", () => {
	...
	assert.match(PAGE_SOURCE, /useJiraGoldenJourneysV5List/u);
	...
	assert.match(LIST_HOOK_SOURCE, /agentCatalog: JIRA_GOLDEN_JOURNEYS_V5_PAY_BOARD_AGENTS/u);
	assert.match(LIST_HOOK_SOURCE, /statusOptions: JIRA_GOLDEN_JOURNEYS_V5_LIST_STATUS_OPTIONS/u);
