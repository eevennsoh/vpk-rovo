...
import { renderJiraGoldenJourneysV5AgentActivityIndicator } from "./data/agent-activity-indicators";
import {
...
import { useJiraGoldenJourneysV5AgentSessionSync } from "./hooks/use-jira-golden-journeys-v5-agent-session-sync";
import { useJiraGoldenJourneysV5GenerativeActions } from "./hooks/use-jira-golden-journeys-v5-generative-actions";
import { useJiraGoldenJourneysV5List } from "./hooks/use-jira-golden-journeys-v5-list";

const JIRA_LIST_PANEL_END_GAP_PX = 24;

export default function JiraGoldenJourneysV5Page(): React.ReactElement {
	return (
		<RovoChatProvider agentProfiles={JGP_CHAT_AGENT_PROFILES}>
			<JiraGoldenJourneysV5App />
		</RovoChatProvider>
	);
}

function JiraGoldenJourneysV5App(): React.ReactElement {
	const { chatContextBar, externalThinkingMessageId, openAgentChat } = useJgpAgentChatDemo();
	const [boardColumns, setBoardColumns] = useState(createJiraGoldenJourneysV5PayBoardColumns);
	const {
		composerPrefillRequest,
		handleCardGenerativeActionSubmit,
		handleComposerPrefillConsumed,
	} =
		useJiraGoldenJourneysV5GenerativeActions({
			openAgentChat,
			setBoardColumns,
		});
	...
	const {
		...
		newAgentSessionIds,
		syncedAgentSessions,
	} = useJiraGoldenJourneysV5AgentSessionSync({ active: showBoardContent });
	...
	const handleAgentSessionUnlink = useCallback((session: { id: string }, card: JiraKanbanCardData) => {
		...
		if (!activity) return;
		const detachedSession = toJiraGoldenJourneysV5DetachedAgentSession(activity, card);
			...
			>
				<div className="h-full min-h-0 min-w-0 overflow-hidden bg-surface [&>div]:min-h-0">
					<ExperimentalJiraKanbanPage
						...
						agentSessionAssigneeIdAliases={JIRA_GOLDEN_JOURNEYS_V5_PAY_SESSION_MEMBER_ID_BY_ASSIGNEE_ID}
						agentSessionPresentation={designVariants.panel ? "panel" : "column"}
						columnChrome={designVariants.simpleKanban ? "simple" : "default"}
						agents={JIRA_GOLDEN_JOURNEYS_V5_PAY_BOARD_AGENTS}
						...
						headerAssignees={JIRA_GOLDEN_JOURNEYS_V5_PAY_HEADER_ASSIGNEES}
						...
						renderAgentActivityIndicator={renderJiraGoldenJourneysV5AgentActivityIndicator}
