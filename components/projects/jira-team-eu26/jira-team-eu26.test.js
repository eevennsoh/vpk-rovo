...
test("the route renders the Payments board directly inside Jira app chrome", () => {
	...
	assert.match(PAGE_SOURCE, /<AppLayout[\s\S]*defaultSidebarOpen=\{true\}[\s\S]*hideSettings[\s\S]*product="jira"/u);
...
test("chin-row agent activity indicators use the Team EU renderer", () => {
	...
	assert.match(
		PAGE_SOURCE,
		/import \{ renderJiraTeamEu26AgentActivityIndicator \} from "\.\/data\/agent-activity-indicators";/u,
	);
	assert.match(
		PAGE_SOURCE,
		/<ExperimentalJiraKanbanPage[\s\S]*renderAgentActivityIndicator=\{renderJiraTeamEu26AgentActivityIndicator\}/u,
	);
