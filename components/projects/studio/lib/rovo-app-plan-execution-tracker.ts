...
import type { RovoAppTodoProgressItem } from "@/components/projects/studio/lib/rovo-app-update-todo-progress";
import {
	rovoAppPlanTaskIdsMatch,
	getLatestRovoAppTodoProgressFromMessages,
} from "@/components/projects/studio/lib/rovo-app-plan-task-labels";
...
export interface RovoAppPlanExecutionStatusGroups {
	done: RovoAppPlanExecutionProgressTask[];
	inReview: RovoAppPlanExecutionProgressTask[];
	inProgress: RovoAppPlanExecutionProgressTask[];
	failed: RovoAppPlanExecutionProgressTask[];
	todo: RovoAppPlanExecutionProgressTask[];
}

export interface RovoAppPlanExecutionTrackerViewModel {
	planKey: string;
	planTitle: string;
	...
	agentCount: number;
	taskCount: number;
	taskStatusGroups: RovoAppPlanExecutionStatusGroups;
}
...
function buildPlanTaskRecord(task: ParsedPlanTask): RovoAppTodoProgressItem {
	return {
		id: task.id,
		content: task.label,
		label: task.label,
...
): RovoAppTodoProgressItem & { agentName?: string } {
	return {
...
function mergeTodoItemsWithPlanTasks(
	...
	snapshot: ReturnType<typeof getLatestRovoAppTodoProgressFromMessages>,
): Array<RovoAppTodoProgressItem & { agentName?: string }> {
	...
	const mergedItems: Array<RovoAppTodoProgressItem & { agentName?: string }> = [];
...
function toProgressTask(
	item: RovoAppTodoProgressItem & { agentName?: string },
	thinkingStatusContent: string | null,
): RovoAppPlanExecutionProgressTask {
	const description =
		item.status === "pending"
...
function buildStatusGroups(
	items: ReadonlyArray<RovoAppTodoProgressItem & { agentName?: string }>,
	thinkingStatusContent: string | null,
): RovoAppPlanExecutionStatusGroups {
	return items.reduce<RovoAppPlanExecutionStatusGroups>(
		(result, item) => {
...
export function buildRovoAppPlanExecutionDismissKey(
	threadId: string,
	planKey: string,
): string {
...
export function clearRovoAppPlanExecutionDismissalsForThread(
...
export function resolveRovoAppPlanExecutionTracker(input: {
	activeRun: RovoAppActiveRun | null;
	messages: ReadonlyArray<RovoUIMessage>;
	threadId: string | null;
	threadUpdatedAt?: string | null;
}): RovoAppPlanExecutionTrackerViewModel | null {
	...
	const latestSnapshot = getLatestRovoAppTodoProgressFromMessages(executionMessages);
