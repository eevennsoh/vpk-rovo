...
} from "@/components/ui-audio/hooks/use-transcript-viewer"
...
function useTranscriptViewerContext() {
  const context = use(TranscriptViewerContext)
  if (!context) {
    throw new Error(
...
function TranscriptViewerProvider({
  ...
  return (
    <TranscriptViewerContext value={value}>
      {children}
    </TranscriptViewerContext>
  )
}
	...
	return (
		<span
			...
			className={cn(
				...
				status === "unspoken" && "text-text-subtle",
				status === "current" && "bg-bg-selected-bold text-text-inverse",
				className
			)}
			{...props}
    >
