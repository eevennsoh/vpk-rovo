...
const sidebarNavItemVariants = cva("group/sidebar-nav-item relative flex min-h-8 w-full min-w-0 items-center gap-1 rounded-md p-1 transition-colors duration-fast ease-out", {
	variants: {
		interactionState: {
			rest: "",
		...
		selected: {
			false: "bg-transparent hover:bg-bg-neutral-subtle-hovered active:bg-bg-neutral-subtle-pressed",
			true: "bg-bg-selected hover:bg-bg-selected-hovered active:bg-bg-selected-pressed",
		},
	},
	defaultVariants: {
...
function normalizeIconNode(node: React.ReactNode, size: "small" | "medium"): React.ReactElement {
	if (React.isValidElement(node)) {
		return React.cloneElement(node as React.ReactElement<{ label?: string; size?: "small" | "medium" }>, {
			label: "",
			size,
		});
	}
		...
		>
			<span
				...
				className={cn(
					...
					interactionState === "focus-visible" && "border-ring ring-3 ring-ring/50 opacity-100",
				)}
			/>
			...
			>
				...
				<span data-slot="sidebar-nav-item-content" className="min-w-0 flex-1 pl-0.5">
					...
					{description ? (
						<span data-slot="sidebar-nav-item-description" className="block truncate text-xs text-text-subtlest">
							{description}
						</span>
					) : null}
				</span>
			...
			{meta ? (
				<div data-slot="sidebar-nav-item-meta" className="flex shrink-0 items-center overflow-hidden">
					{meta}
				</div>
			) : null}
...
export interface SidebarNavItemActionProps extends React.ComponentProps<"button"> {
	"aria-label": string;
}

function SidebarNavItemAction({ className, children, type = "button", ...props }: Readonly<SidebarNavItemActionProps>) {
	return (
		<button
			data-slot="button"
			type={type}
			...
			{...props}
		>
			<Icon aria-hidden label="" render={normalizeIconNode(children, "small")} className="size-3 [&_svg]:size-3" />
		</button>
	);
}
...
export { SidebarNavItem, SidebarNavItemAction, SidebarNavItemCount };
