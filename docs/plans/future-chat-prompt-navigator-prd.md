# Future Chat prompt navigator PRD

This PRD defines a minimal in-thread navigation feature for `future-chat`.
...
- **Problem statement**: In long `/future-chat` threads, users must manually
...
- **Proposed solution**: Add a minimal in-thread navigation control to the
  `future-chat` message pane. Hovering the control opens a compact overlay,
...
- **User personas**:
  - Power users working in long `future-chat` threads on desktop.
  - Users who frequently revisit earlier prompts to refine, repeat, or
    reference prior instructions.
- **User stories**:
  - As a `future-chat` user, I want a compact in-thread navigation control so
    that I can jump to earlier prompts without visually cluttering the chat.
  - As a `future-chat` user, I want to preview short snippets of my earlier
    prompts so that I can identify the right message before navigating.
  - As a `future-chat` user, I want clicking a snippet to scroll me to that
    exact message so that I can resume work faster.
- **Acceptance criteria**:
  - The feature appears only in `/future-chat`.
...
This section maps the feature onto the existing `future-chat` implementation
so the work can stay frontend-only and local to the active thread surface.

- **Architecture overview**:
  - Source message data comes from
    `components/projects/future-chat/hooks/use-future-chat.ts` and already
    flows into the `future-chat` shell and message surface.
  ...
  - User message nodes expose a stable DOM hook such as
    `data-future-chat-message-id="{message.id}"` so the navigator can scroll
