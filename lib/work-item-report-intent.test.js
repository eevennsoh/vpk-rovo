...
test("detects clear work-item vpk-html artifact generation requests", () => {
	const positivePrompts = [
		...
		"create an offline HTML one-pager for this work item",
		"draft a brief for the current ticket",
		"turn this Jira issue into a memo",
		"build a self-contained HTML document from this",
		"render an HTML artifact for the open work item",
		"/vpk-html generate a report",
	];

	for (const prompt of positivePrompts) {
		assert.equal(isWorkItemReportIntent(prompt), true, prompt);
		assert.equal(isVpkHtmlArtifactIntent(prompt), true, prompt);
	}
});

test("avoids ordinary questions, field references, and non-artifact requests", () => {
	const negativePrompts = [
		...
		"how do I create a one-pager for this work item?",
		"create a Jira ticket for this work",
		"summarize this ticket in chat",
	];

	for (const prompt of negativePrompts) {
		assert.equal(isWorkItemReportIntent(prompt), false, prompt);
		assert.equal(isVpkHtmlArtifactIntent(prompt), false, prompt);
	}
});
...
test("builds the vpk-html routing block for artifact intent", () => {
	const contextBlock = buildWorkItemReportRequestContext({
		contextDescription: ACTIVE_CONTEXT,
		promptText: "create an offline HTML one-pager for this work item",
		skillId: "vpk-html",
	});
...
test("resolves artifact routing only when context is available", () => {
	assert.deepEqual(
		resolveWorkItemReportRequest({
			contextDescription: ACTIVE_CONTEXT,
			promptText: "generate an offline HTML brief for this work item",
