...
const DEEP_PLAN_INSTRUCTION = [
	...
	"4a. If you use `invoke_subagents` or `create_technical_plan` during plan mode, you must still continue the same planning turn until you call `exit_plan_mode`. Take the output from `create_technical_plan` and pass it (reformatted to match the template in rule 6) as the markdown argument to `exit_plan_mode`. Do not stop after subagent exploration or plan generation. Do not end the turn without calling `exit_plan_mode`.",
	...
	"5a. Post-approval task setup: After approval switches you to `default` mode, your FIRST action must be calling `update_todo` to create a structured checklist from the plan tasks. Use the same task labels and dependency prefixes from the plan. Do not begin implementation until `update_todo` completes.",
	"6. Plan structure: The markdown passed to `exit_plan_mode` must follow this template exactly:",
	"",
	"   ```",
	...
	"",
	"   ## Scope",
	"   - In: <comma-separated list of what is in scope>",
	"   - Out: <what is explicitly not in scope>",
	"",
	"   ## Action items",
	"   1. <Verb-first step, mentioning files/modules> — <2-3 sentence description with acceptance criteria>",
	"   2. <Step 2> — <description>",
	"   3. [needs 1] <Step 3, depends on step 1> — <description>",
	"   4. [needs 1,2] <Step 4, depends on steps 1 and 2> — <description>",
	"   ...",
	"   (target 6-10 items, ordered: discovery → changes → tests → rollout)",
	"",
	"   ## Open questions",
	"   - <Any remaining unknowns, max 3>",
	"",
	"   ```mermaid",
	"   graph TD",
	"     task1[\"Step 1 label\"]",
	...
	"   - Each item must be atomic and independently executable",
	"   - Point to likely files/modules (e.g. `src/...`, `components/...`)",
	"   - Include at least one item for tests/validation and one for edge cases/risk",
	"   - Verb-first: \"Add…\", \"Refactor…\", \"Create…\", \"Verify…\"",
	"   - Avoid vague steps (\"handle backend\") or code snippets",
	"   - Use dependency prefixes for blocked tasks: `[needs <id,id>]` (e.g. `[needs 1,2] Wire API`). Leave independent tasks without a `[needs]` prefix.",
	"   - Include a Mermaid `graph TD` diagram at the end showing task dependency edges. Only draw edges for actual dependencies — independent tasks have no incoming edges.",
