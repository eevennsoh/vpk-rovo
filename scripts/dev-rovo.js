...
const { getRovoBasePort } = require("./lib/worktree-ports");
const { isPortAvailable, checkRovoHealth } = require("./lib/rovo-utils");
const {
	buildSpawnArgsForPort,
	prepareRovoRuntime,
} = require("./lib/rovo-runtime");
const { startSupervisedRovoPorts } = require("./lib/rovo-supervisor");
const { getRecordedPoolDecision } = require("./lib/rovo-recorded-pool");

const basePort = getRovoBasePort();
const portFile = path.join(process.cwd(), ".dev-rovo-port");
const portsFile = path.join(process.cwd(), ".dev-rovo-ports");
...
/**
 * Gracefully stop all listeners in the candidate Rovo port range.
...
const cleanupAllInstances = async (minPort, maxPort) => {
	console.log(`[rovo] Force clean start enabled. Stopping listeners on ports ${minPort}-${maxPort}...`);
	...
	console.log(`[rovo] Sent SIGTERM to ${signalledPids.size} process(es). Waiting for graceful shutdown...`);
	await sleep(2000);
	...
	if (forceKilled > 0) {
		console.log(`[rovo] Force-killed ${forceKilled} process(es) that didn't exit gracefully.`);
		await sleep(250);
	}
};

/**
 * Gracefully stop stale Rovo instances on unhealthy ports.
...
const cleanupStaleInstances = async (minPort, maxPort) => {
	console.log(`[rovo] Scanning for stale instances (ports ${minPort}-${maxPort})...`);
	const stalePids = new Set();

	for (let port = minPort; port <= maxPort; port++) {
		const health = await checkRovoHealth(port);
		...
		if (!health.healthy) {
			console.log(
				`[rovo] Found unhealthy instance on port ${port} (status: ${health.status}). Stopping...`
			);
			const pids = getListeningPids(port);
			for (const pid of pids) {
	...
	console.log(`[rovo] Sent SIGTERM to ${stalePids.size} stale process(es). Waiting for graceful shutdown...`);
	await sleep(2000);
...
const readRecordedPorts = (maxTries) => {
	...
	if (fs.existsSync(portsFile)) {
		try {
			...
			console.log(
				`[rovo] Ignoring stale port pool file; expected ports in range ${basePort}-${basePort + maxTries - 1}.`
			);
			cleanup();
		} catch {
			// Ignore parse errors
	...
	if (fs.existsSync(portFile)) {
		try {
			...
			console.log(
				`[rovo] Ignoring stale port file (${port}); expected range ${basePort}-${basePort + maxTries - 1}.`
			);
			cleanup();
		} catch {
			// Ignore read errors
...
const run = async () => {
	...
	const poolSize = Math.max(1, Number.parseInt(process.env.ROVO_POOL_SIZE ?? "1", 10));
	const forceCleanStart = process.env.ROVO_FORCE_CLEAN_START === "true";
	...
	if (recordedPorts !== null) {
		...
		const healthChecks = allInUse
			? await Promise.all(
				recordedPorts.map(async (p) => {
					const health = await checkRovoHealth(p);
		...
		if (recordedPoolDecision.action === "reuse") {
			...
			console.log(
				`Rovo serve pool already running on ports ${recordedPorts.join(", ")}. Reusing existing processes.`
			);
			return;
		}
		...
		if (recordedPoolDecision.reason === "pool-size-mismatch") {
			console.log(
				`[rovo] Existing pool size (${recordedPoolDecision.recordedPoolSize}) does not match requested pool size (${recordedPoolDecision.requestedPoolSize}). Restarting...`
			);
			await cleanupAllInstances(basePort, scanMax);
		} else if (recordedPoolDecision.reason === "recorded-pool-unhealthy") {
			console.log("[rovo] Some existing instances are unhealthy. Restarting...");
		} else if (recordedPoolDecision.reason === "recorded-ports-not-running") {
			console.log("[rovo] Recorded Rovo ports are no longer running. Restarting...");
		}
	...
	if (ports.length === 1) {
		...
		console.log(`[rovo] Pool of ${ports.length} ports: ${ports.join(", ")}`);
	}
