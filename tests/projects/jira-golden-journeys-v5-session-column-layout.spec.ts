...
const JIRA_GOLDEN_JOURNEYS_V5_EMBEDDED_URL = (
	process.env.PLAYWRIGHT_BASE_URL ?? "http://127.0.0.1:3000"
) + "/preview/projects/jira-golden-journeys-v5?embedded=1";
...
test("hovering the leading gutter stays open without bouncing under a stationary pointer", async ({ page }) => {
	await page.goto(JIRA_GOLDEN_JOURNEYS_V5_EMBEDDED_URL, { waitUntil: "domcontentloaded" });
	await expect(page.getByRole("heading", { name: "Jira Design" })).toBeVisible();
